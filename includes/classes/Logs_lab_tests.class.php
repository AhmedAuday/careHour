<?php
  ...
  class Logs_lab_tests extends Database {
    ...
    public function __construct($id = null){
      ...
      if($id != null){
        ...
        $sql = "SELECT * FROM `logs_lab_tests` WHERE `id`=$this->id";
    ...
    public function getAll($limit = 1000000){
      ...
      $sql = "SELECT * FROM `logs_lab_tests` WHERE $this->sqlCondition ORDER BY `id` LIMIT $limit";
      $result = $this->db()->query($sql);
    ...
    public function getAllReversed($limit = 1000000){
      if(!is_numeric($limit)){ return '[]'; }
      $sql = "SELECT * FROM `logs_lab_tests` WHERE $this->sqlCondition ORDER BY `id` DESC LIMIT $limit";
      $result = $this->db()->query($sql);
      $data = $result->fetchAll();
      $json_data = json_encode($data);
      $json_data = json_decode($json_data);
    ...
    public function getNumberOfRows(){
      $sql = "SELECT COUNT(*) AS 'numberOfRows' FROM `logs_lab_tests` WHERE $this->sqlCondition";
      $result = $this->db()->query($sql);
      $row = $result->fetch();
    ...
    public function getByActionn(){
      $sql = "SELECT * FROM `logs_lab_tests` WHERE `actionn`='$this->actionn'";
      $result = $this->db()->query($sql);
      if($result->rowCount() > 0){
        $row = $result->fetch();
        $this->id = $row['id'];
    ...
    public function getByNew_doctor_id(){
      $sql = "SELECT * FROM `logs_lab_tests` WHERE `new_doctor_id`='$this->new_doctor_id'";
      $result = $this->db()->query($sql);
      if($result->rowCount() > 0){
        $row = $result->fetch();
        $this->id = $row['id'];
    ...
    public function getByNew_patient_id(){
      $sql = "SELECT * FROM `logs_lab_tests` WHERE `new_patient_id`='$this->new_patient_id'";
      $result = $this->db()->query($sql);
      if($result->rowCount() > 0){
        $row = $result->fetch();
        $this->id = $row['id'];
    ...
    public function getByNew_test_name(){
      $sql = "SELECT * FROM `logs_lab_tests` WHERE `new_test_name`='$this->new_test_name'";
      $result = $this->db()->query($sql);
      if($result->rowCount() > 0){
        $row = $result->fetch();
        $this->id = $row['id'];
    ...
    public function getByNew_test_results(){
      $sql = "SELECT * FROM `logs_lab_tests` WHERE `new_test_results`='$this->new_test_results'";
      $result = $this->db()->query($sql);
      if($result->rowCount() > 0){
        $row = $result->fetch();
        $this->id = $row['id'];
    ...
    public function getByNew_test_date(){
      $sql = "SELECT * FROM `logs_lab_tests` WHERE `new_test_date`='$this->new_test_date'";
      $result = $this->db()->query($sql);
      if($result->rowCount() > 0){
        $row = $result->fetch();
        $this->id = $row['id'];
    ...
    public function getByLogs_cdate(){
      $sql = "SELECT * FROM `logs_lab_tests` WHERE `logs_cdate`='$this->logs_cdate'";
      $result = $this->db()->query($sql);
      if($result->rowCount() > 0){
        $row = $result->fetch();
        $this->id = $row['id'];
    ...
    public function getByLogs_ctime(){
      $sql = "SELECT * FROM `logs_lab_tests` WHERE `logs_ctime`='$this->logs_ctime'";
      $result = $this->db()->query($sql);
      if($result->rowCount() > 0){
        $row = $result->fetch();
        $this->id = $row['id'];
    ...
    public function getByLog_ByUser(){
      $sql = "SELECT * FROM `logs_lab_tests` WHERE `log_ByUser`='$this->log_ByUser'";
      $result = $this->db()->query($sql);
      if($result->rowCount() > 0){
        $row = $result->fetch();
        $this->id = $row['id'];
    ...
    public function getByTimee(){
      $sql = "SELECT * FROM `logs_lab_tests` WHERE `timee`='$this->timee'";
      $result = $this->db()->query($sql);
      if($result->rowCount() > 0){
        $row = $result->fetch();
        $this->id = $row['id'];
    ...
    public function getByTime(){
      $sql = "SELECT * FROM `logs_lab_tests` WHERE `time`='$this->time'";
      $result = $this->db()->query($sql);
      if($result->rowCount() > 0){
        $row = $result->fetch();
        $this->id = $row['id'];
    ...
    public function getLastRow(){
      $sql = "SELECT * FROM `logs_lab_tests` WHERE $this->sqlCondition ORDER BY `id` DESC";
      $result = $this->db()->query($sql);
      if($result->rowCount() > 0){
        $row = $result->fetch();
        $this->id = $row['id'];
    ...
    public function getFirstRow(){
      $sql = "SELECT * FROM `logs_lab_tests` WHERE $this->sqlCondition ORDER BY `id`";
      $result = $this->db()->query($sql);
      if($result->rowCount() > 0){
        $row = $result->fetch();
        $this->id = $row['id'];
    ...
    public function add(){
      $sql = "INSERT INTO `logs_lab_tests` (`id`, `actionn`, `new_doctor_id`, `new_patient_id`, `new_test_name`, `new_test_results`, `new_test_date`, `logs_cdate`, `logs_ctime`, `log_ByUser`, `timee`, `time`) VALUES 
      ('$this->id', '$this->actionn', '$this->new_doctor_id', '$this->new_patient_id', '$this->new_test_name', '$this->new_test_results', '$this->new_test_date', '$this->logs_cdate', '$this->logs_ctime', '$this->log_ByUser', '$this->timee', '$this->time')";
      if($this->db()->query($sql)){
        return true;
      }else{
        return false;
    ...
    public function delete() {
      $sql = "DELETE FROM `logs_lab_tests` WHERE `id`='$this->id'";
      if($this->db()->query($sql)){
        return true;
      }else{
        return false;
    ...
    public function giveIdentity($days){
      setcookie('logs_lab_tests', encode("$this->id"), time() + (86400 * $days), "/");
    }

    public function getIdentity($days){
      if(isset($_COOKIE['logs_lab_tests'])){
        decode($_COOKIE['logs_lab_tests']);
      }else{return false;}
    }

    public function deleteIdentity($days){
      setcookie("logs_lab_tests", '', time() - 3600);
    }

    public function giveAuthority(){
      $_SESSION['logs_lab_tests'] = encrypt($this->id);
      $_SESSION['logs_lab_tests_id'] = $this->id;
    }

    public function getAuthority(){
      if(isset($_SESSION['logs_lab_tests']) && isset($_SESSION['logs_lab_tests_id'])){
        $tmp_id = filter($_SESSION['logs_lab_tests_id']);
        $sql = "SELECT * from `logs_lab_tests` WHERE `id`='$tmp_id'";
        $result = $this->db()->query($sql);
        $row = $result->fetch();
        if(encrypt($row['id']) == $_SESSION['logs_lab_tests']){
          return $_SESSION['logs_lab_tests_id'];
        }else{return -2;}
      }else{return -1;}
    }
