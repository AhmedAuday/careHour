<?php
  ...
  class Logs_patient_notes extends Database {
    ...
    public function __construct($id = null){
      ...
      if($id != null){
        ...
        $sql = "SELECT * FROM `logs_patient_notes` WHERE `id`=$this->id";
    ...
    public function getAll($limit = 1000000){
      ...
      $sql = "SELECT * FROM `logs_patient_notes` WHERE $this->sqlCondition ORDER BY `id` LIMIT $limit";
      $result = $this->db()->query($sql);
    ...
    public function getAllReversed($limit = 1000000){
      if(!is_numeric($limit)){ return '[]'; }
      $sql = "SELECT * FROM `logs_patient_notes` WHERE $this->sqlCondition ORDER BY `id` DESC LIMIT $limit";
      $result = $this->db()->query($sql);
      $data = $result->fetchAll();
      $json_data = json_encode($data);
      $json_data = json_decode($json_data);
    ...
    public function getNumberOfRows(){
      $sql = "SELECT COUNT(*) AS 'numberOfRows' FROM `logs_patient_notes` WHERE $this->sqlCondition";
      $result = $this->db()->query($sql);
      $row = $result->fetch();
    ...
    public function getByActionn(){
      $sql = "SELECT * FROM `logs_patient_notes` WHERE `actionn`='$this->actionn'";
      $result = $this->db()->query($sql);
      if($result->rowCount() > 0){
        $row = $result->fetch();
        $this->id = $row['id'];
    ...
    public function getByNew_patient_id(){
      $sql = "SELECT * FROM `logs_patient_notes` WHERE `new_patient_id`='$this->new_patient_id'";
      $result = $this->db()->query($sql);
      if($result->rowCount() > 0){
        $row = $result->fetch();
        $this->id = $row['id'];
    ...
    public function getByNew_notes(){
      $sql = "SELECT * FROM `logs_patient_notes` WHERE `new_notes`='$this->new_notes'";
      $result = $this->db()->query($sql);
      if($result->rowCount() > 0){
        $row = $result->fetch();
        $this->id = $row['id'];
    ...
    public function getByNew_created_at(){
      $sql = "SELECT * FROM `logs_patient_notes` WHERE `new_created_at`='$this->new_created_at'";
      $result = $this->db()->query($sql);
      if($result->rowCount() > 0){
        $row = $result->fetch();
        $this->id = $row['id'];
    ...
    public function getByLogs_cdate(){
      $sql = "SELECT * FROM `logs_patient_notes` WHERE `logs_cdate`='$this->logs_cdate'";
      $result = $this->db()->query($sql);
      if($result->rowCount() > 0){
        $row = $result->fetch();
        $this->id = $row['id'];
    ...
    public function getByLogs_ctime(){
      $sql = "SELECT * FROM `logs_patient_notes` WHERE `logs_ctime`='$this->logs_ctime'";
      $result = $this->db()->query($sql);
      if($result->rowCount() > 0){
        $row = $result->fetch();
        $this->id = $row['id'];
    ...
    public function getByLog_ByUser(){
      $sql = "SELECT * FROM `logs_patient_notes` WHERE `log_ByUser`='$this->log_ByUser'";
      $result = $this->db()->query($sql);
      if($result->rowCount() > 0){
        $row = $result->fetch();
        $this->id = $row['id'];
    ...
    public function getByTimee(){
      $sql = "SELECT * FROM `logs_patient_notes` WHERE `timee`='$this->timee'";
      $result = $this->db()->query($sql);
      if($result->rowCount() > 0){
        $row = $result->fetch();
        $this->id = $row['id'];
    ...
    public function getByTime(){
      $sql = "SELECT * FROM `logs_patient_notes` WHERE `time`='$this->time'";
      $result = $this->db()->query($sql);
      if($result->rowCount() > 0){
        $row = $result->fetch();
        $this->id = $row['id'];
    ...
    public function getLastRow(){
      $sql = "SELECT * FROM `logs_patient_notes` WHERE $this->sqlCondition ORDER BY `id` DESC";
      $result = $this->db()->query($sql);
      if($result->rowCount() > 0){
        $row = $result->fetch();
        $this->id = $row['id'];
    ...
    public function getFirstRow(){
      $sql = "SELECT * FROM `logs_patient_notes` WHERE $this->sqlCondition ORDER BY `id`";
      $result = $this->db()->query($sql);
      if($result->rowCount() > 0){
        $row = $result->fetch();
        $this->id = $row['id'];
    ...
    public function add(){
      $sql = "INSERT INTO `logs_patient_notes` (`id`, `actionn`, `new_patient_id`, `new_notes`, `new_created_at`, `logs_cdate`, `logs_ctime`, `log_ByUser`, `timee`, `time`) VALUES 
      ('$this->id', '$this->actionn', '$this->new_patient_id', '$this->new_notes', '$this->new_created_at', '$this->logs_cdate', '$this->logs_ctime', '$this->log_ByUser', '$this->timee', '$this->time')";
      if($this->db()->query($sql)){
        return true;
      }else{
        return false;
    ...
    public function update(){
      $sql = "UPDATE `logs_patient_notes` SET `id`='$this->id', `actionn`='$this->actionn', `new_patient_id`='$this->new_patient_id', `new_notes`='$this->new_notes', `new_created_at`='$this->new_created_at', `logs_cdate`='$this->logs_cdate', `logs_ctime`='$this->logs_ctime', `log_ByUser`='$this->log_ByUser', `timee`='$this->timee', `time`='$this->time' WHERE `id`='$this->id'";
      if($this->db()->query($sql)){
        return true;
      }else{
        return false;
    ...
    public function delete() {
      $sql = "DELETE FROM `logs_patient_notes` WHERE `id`='$this->id'";
      if($this->db()->query($sql)){
        return true;
      }else{
        return false;
    ...
    public function giveIdentity($days){
      setcookie('logs_patient_notes', encode("$this->id"), time() + (86400 * $days), "/");
    }

    public function getIdentity($days){
      if(isset($_COOKIE['logs_patient_notes'])){
        decode($_COOKIE['logs_patient_notes']);
      }else{return false;}
    }

    public function deleteIdentity($days){
      setcookie("logs_patient_notes", '', time() - 3600);
    }

    public function giveAuthority(){
      $_SESSION['logs_patient_notes'] = encrypt($this->id);
      $_SESSION['logs_patient_notes_id'] = $this->id;
    }

    public function getAuthority(){
      if(isset($_SESSION['logs_patient_notes']) && isset($_SESSION['logs_patient_notes_id'])){
        $tmp_id = filter($_SESSION['logs_patient_notes_id']);
        $sql = "SELECT * from `logs_patient_notes` WHERE `id`='$tmp_id'";
        $result = $this->db()->query($sql);
        $row = $result->fetch();
        if(encrypt($row['id']) == $_SESSION['logs_patient_notes']){
          return $_SESSION['logs_patient_notes_id'];
        }else{return -2;}
      }else{return -1;}
    }
