<?php
  ...
  class Appointments extends Database {
    ...
    public function getByOn_date(){
      $sql = "SELECT * FROM `appointments` WHERE `on_date`='$this->on_date'";
      $result = $this->db()->query($sql);
      if($result->rowCount() > 0){
        $row = $result->fetch();
        $this->id = $row['id'];
    ...
    public function add(){
      $sql = "INSERT INTO `appointments` (`id`, `doctor_id`, `patient_id`, `appointment_date`, `appointment_type`, `notes`, `timee`, `on_date`, `time`) VALUES 
      ('$this->id', '$this->doctor_id', '$this->patient_id', '$this->appointment_date', '$this->appointment_type', '$this->notes', '$this->timee', '$this->on_date', '$this->time')";
      if($this->db()->query($sql)){
        return true;
      }else{
        return false;
    ...
    public function update(){
      $sql = "UPDATE `appointments` SET `id`='$this->id', `doctor_id`='$this->doctor_id', `patient_id`='$this->patient_id', `appointment_date`='$this->appointment_date', `appointment_type`='$this->appointment_type', `notes`='$this->notes', `timee`='$this->timee', `on_date`='$this->on_date', `time`='$this->time' WHERE `id`='$this->id'";
      if($this->db()->query($sql)){
        return true;
      }else{
        return false;
